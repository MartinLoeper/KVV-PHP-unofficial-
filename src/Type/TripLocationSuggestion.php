...
class TripLocationSuggestion {
	...
	public function __construct($requestInfo, $sessionID, $requestID, $origin_stations, $destination_stations) {
	...
	/**
		...
		@param $efa a object of type \KVV\EFA to use for the query
		@param $timestamp the timestamp which should be used for the search
	...
	*/
	public function utilize(\KVV\EFA &$efa, $timestamp, $start, $end) {
		if($start >= count($this->origin_stations))
			throw new \Exception("Index Out Of Bounds!", 500);
			
		if($end >= count($this->destination_stations))
			throw new \Exception("Index Out Of Bounds!", 500);
		
		$efa->setSessionId($this->getSessionId());
		$efa->setRequestId($this->getRequestId());

		return $efa->search($this->origin_stations[$start], $this->destination_stations[$end], $timestamp);
	}
}
?>